...
def processRule(m, name):
    ...
    if len(result[0]) > 0 and len(result[1]) == 0:
        ...
        name_list = [
            result[0][0] + "".join([word for word in result[1]]),
        ...
        name_list = [
            "".join([word for word in result[1]]),
...
请选择字典模式：
    ...
    if model == 1:
        ...
        with open(os.path.join(path, "default.txt"), "w") as f:
            for i in dicts:
                f.write(i + "\n")
        f.close()
    elif model == 2:
        ...
        dicts = sorted(dicts, key=lambda i: len(i), reverse=False)
        print(f"[+] 成功生成共计{len(dicts)}条密码！！请查看！")
        with open(os.path.join(path, f"{keyword}.txt"), "w") as f:
            for i in dicts:
                f.write(i + "\n")
        f.close()
    elif model == 3:
    ...
    请选择生成模式：
        ...
        with open(os.path.join(path, "process_usernames.txt"), "w") as f:
            for i in users:
                f.write(i + "\n")
        f.close()
        with open(os.path.join(path, "process_passwords.txt"), "w") as fp:
            for i in passwords:
                fp.write(i + "\n")
        fp.close()
        print(
            f"[+] 成功为{len(set(users))}个用户名生成共计{len(passwords)}条密码！！请查看！"
        )
